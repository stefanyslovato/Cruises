...
import './DropdownDestination.css';
import OnClickOut from "react-onclickoutside";

function DropdownDestination({ title, items = [], multiSelect = false }) {
  const [open, setOpen] = useState(false);
  const [selection, setSelection] = useState([]);
  const toggle = () => setOpen(!open);
  DropdownDestination.handleClickOutside = () => setOpen(false);
  ...
  return (
    <>
      <div className="dd-wrapper">
        <p className="dd-wrapper-title">Cruising to</p>
        <div
          tabIndex={0}
          className="dd-header"
          role="button"
...
const clickOutsideConfig = {
    handleClickOutside: () => DropdownDestination.handleClickOutside,
}

export default OnClickOut(DropdownDestination, clickOutsideConfig);

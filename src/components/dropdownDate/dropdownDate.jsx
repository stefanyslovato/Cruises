...
function DropdownDate({ title, items = [], multiSelect = false }) {
  const [open, setOpen] = useState(false);
  const [selection, setSelection] = useState([]);
  const toggle = () => setOpen(!open);
  DropdownDate.handleClickOutside = () => setOpen(false);
  ...
  return (
    <>
      <div className="dd-wrapper">
        <p className="dd-wrapper-title">Leaving</p>
        <div
          tabIndex={0}
          className="dd-header"
          role="button"
...
const clickOutsideConfig = {
    handleClickOutside: () => DropdownDate.handleClickOutside,
}

export default OnClickOut(DropdownDate, clickOutsideConfig);
